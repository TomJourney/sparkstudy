 ...
 */
public class SparkGuideChapter0401 {
    ...
    public static void main(String[] args) {
        ...
        try {
            new SparkGuideChapter0401().execute(sparkSessionWrapper);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    private void execute(SparkSessionWrapper sparkSessionWrapper) {
        Dataset<Row> dataset = sparkSessionWrapper.getSparkSession().range(500).toDF("number");
        dataset.show();
    }
}
